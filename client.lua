	...
	while true do 
		...
		if #offers > 0 then 
			...
			if label == 'Money' then 
				...
				Draw2DText(Teko, .5, .5, 'You have a donator GEMS to claim!', 1.0);
				Draw2DText(Teko, .5, .55, 'Press ARROW_UP to claim or ARROW_DOWN to reject...', 1.0);
			end
			if label == 'Job' then 
				local nameLab = offers[1][2];
				local jobName = offers[1][3];
				local jobGrade = offers[1][4];
				Draw2DText(Teko, .5, .5, 'You have a donator JOB to claim!', 1.0);
				Draw2DText(Teko, .5, .55, 'Press ARROW_UP to claim or ARROW_DOWN to reject...', 1.0);
			end
			if label == 'Item' then 
				local nameLab = offers[1][2];
				...
				Draw2DText(Teko, .5, .5, 'You have donator ITEM(S) to claim!', 1.0);
				Draw2DText(Teko, .5, .55, 'Press ARROW_UP to claim or ARROW_DOWN to reject...', 1.0);
			end
		end  
	end
